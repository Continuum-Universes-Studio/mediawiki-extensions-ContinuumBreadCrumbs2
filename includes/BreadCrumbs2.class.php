 ...
 * @license CC-BY-3.0
 */
namespace ContinuumUniverses\BreadCrumbs2;
...
class BreadCrumbs2 {
	...
	public function getOutput() {
		...
		if ( class_exists( '\ForumEmotes\Hooks' ) && method_exists( '\ForumEmotes\Hooks', 'replaceTokensInHtmlPreserveText' ) ) {
			try {
				$breadcrumb = \ForumEmotes\Hooks::replaceTokensInHtmlPreserveText( $breadcrumb );
			} catch ( \Throwable $e ) {
				// If emote rendering fails, fall back to the plain breadcrumb HTML.
			}
		}

		return Html::rawElement( 'div', [ 'id' => 'breadcrumbs2' ], $breadcrumb );
	}
	 ...
	 */
	public function __construct( array $categories, Title $title, UserIdentity $user ) {
	...
	/**
	 * Look up the menu corresponding to the first matching category expression from the list
	 *
	 * @param array $categories
	 * @return array
	 */
	function matchFirstCategory( array $categories ) {
		...
		# Look for the first matching category expression or a default string.
		# Category expressions can be a single category, or multiple categories joined
		# with "&&" to require all of them before a breadcrumb is selected.
		foreach ( $matches[1] as $nav ) {
			...
			if ( $pos !== false ) {
				...
				if ( $cat == 'default' ) {
					$breadcrumb = $crumb;
				} elseif ( self::matchesCategoryExpression( $cat, $categories ) ) {
					$breadcrumb = $crumb;
					break;
				}
			}
	...
	/**
	 * Check whether the current page categories satisfy a category expression.
	 * A bare category name matches as before. A compound expression joined with
	 * "&&" requires all listed categories to be present.
	 *
	 * @param string $categoryExpression
	 * @param array $categories
	 * @return bool
	 */
	private static function matchesCategoryExpression( string $categoryExpression, array $categories ): bool {
		$requiredCategories = preg_split( '/\s*&&\s*/', $categoryExpression ) ?: [];
		$requiredCategories = array_values( array_filter( array_map( 'trim', $requiredCategories ), 'strlen' ) );
		if ( !$requiredCategories ) {
			return false;
		}

		foreach ( $requiredCategories as $requiredCategory ) {
			if ( !in_array( $requiredCategory, $categories, true ) ) {
				return false;
			}
		}
